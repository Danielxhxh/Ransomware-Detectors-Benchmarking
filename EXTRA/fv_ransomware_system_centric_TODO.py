...
for session_name in os.listdir(ransomware_logs_path):
    ...
    try:
        if session_name.endswith(".gz"):
            ...
            with gzip.open(session_name, 'r') as fin:
                for line in fin:
                    try:
        ...
        for key, val in features.items():
            with open('/home/dorjan/Desktop/ransomware_detector/ransomware_datasets/ransomware_sys_centric/' + machine_name + '/tier'+str(tier)+'/' + machine_name + '_' + str(key) + '.csv', 'a+') as fp:
                wr = csv.writer(fp)
                wr.writerows(val)

        output_file = FEATURES_PATH / "ransomware_shieldfs_features.csv"
