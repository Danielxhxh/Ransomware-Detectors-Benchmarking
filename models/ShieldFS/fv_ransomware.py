...
ransomware_path = '/home/dhitaj/rw_det/ransomware/'
...
def load_machine_statistics():
    with open('statistics_summary_virtual.txt', 'r') as fp:
        for ln in fp:
            try:
                els = ln.split('\t')
...
print(number_folders, number_files)


for session_name in os.listdir(ransomware_path+machine_name):
    print("The ransomware log is: ", session_name)
    if os.path.isfile(os.path.join(ransomware_path + machine_name, session_name)):
        session_folder_path = os.path.join(ransomware_path + machine_name, session_name)
        current_tick = 0
        features = dict()
        ...
        seen_files = []
        try:
            if session_folder_path.endswith(".gz"):
                ransomware_pid = get_ransomware_pid(session_name+'.csv')
                print("The ransomware pid is: ", ransomware_pid)
                with gzip.open(session_folder_path, 'r') as fin:
                    for line in fin:
                        try:
        ...
        for key, val in features.items():
            with open('datasets/ransomware_process_centric/' + machine_name + '/tier'+str(tier)+'/' + machine_name + '_' + str(key) + '.csv', 'a+') as fp:
                wr = csv.writer(fp)
                wr.writerows(val)
