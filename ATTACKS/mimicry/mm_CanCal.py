...
# --- CONFIGURATION ---
# CanCal Strategy: LOOK LIKE AN INSTALLER.
# Installers create many files but delete few.
# Target Ratio (Delete/Create) ~ 0.5 or lower.
# Real Ransomware Ratio = 1.0 (1 Create : 1 Delete).
# We inject 1 DUMMY CREATE for every encryption event to lower ratio to 0.5.
DUMMY_CREATES_PER_OP = 1 

# Volume Limiter: Don't look like a "Mass Installer" (too fast).
MAX_FILES_PER_WINDOW = 30
# ---------------------
...
def create_mimic_state():
    return {
        ...
        'extensions': set(),
        'ntype_start': 0,
        'files_processed_in_window': 0 # Volume Limiter
    }

def extract_mimicry_cancal_features():
    output_base_path = FEATURES_PATH / "mimicry_CanCal"
    ...
    print(f"[*] Starting CanCal Mimicry Attack (Profile: Installer)")
    print(f"[*] Strategy: Inject {DUMMY_CREATES_PER_OP} dummy create per encryption.")
    print(f"[*] Volume Limit: {MAX_FILES_PER_WINDOW} files per window.")
    
    os.makedirs(output_base_path, exist_ok=True)
    with open(output_file, 'w', newline='') as f:
        # Write Header (Critical for CanCal)
        writer = csv.writer(f)
        writer.writerow(['n_create', 'n_delete', 'n_renamed', 'rtype', 'rtype_change', 
                         'max_n_file', 'n_folder', 'r_file', 'ntype_change', 'Label'])
            ...
            with gzip.open(session_path, 'rt', encoding='utf-8', errors='ignore') as fin:
                ...
                for line in fin:
                    ...
                    try:
                        ...
                        # --- CHECK VOLUME LIMIT ---
                        if state['files_processed_in_window'] >= MAX_FILES_PER_WINDOW:
                            # Start new state (Simulate thread switch or wait)
                            mimic_states.append(create_mimic_state())
                            current_proc_idx += 1
                            state = mimic_states[current_proc_idx]
                            state['previous_time'] = parsed_time
                            state['current_time'] = parsed_time
                            state['ntype_start'] = 0 # New state assumes fresh start
                        ...
                        if major_op == 'IRP_MJ_CREATE':
                            ...
                            state['files_processed_in_window'] += 1
                            change = True
                            ...
                            if param_code == FILE_RENAME_CODE:
                                ...
                                # --- ATTACK: DUMMY INJECTION ---
                                # Inject DUMMY CREATES to dilute the ratio
                                # We add random/temp extensions to look natural
                                for _ in range(DUMMY_CREATES_PER_OP):
                                    state['features']['n_create'] += 1
                                    state['created_exts'].add('.tmp')
                                    state['extensions'].add('.tmp')
                        ...
                        if date_diff_in_seconds(state['current_time'], state['previous_time']) >= TIME_WINDOW:
                            # (Standard CanCal Feature Calculation)
                            ...
                            # CRITICAL: This rtype_change is what we manipulated!
                            # Original: 10 Del / 10 Create = 1.0
                            # Attacked: 10 Del / (10 Real + 10 Dummy) Create = 0.5
                            ...
                            state['files_processed_in_window'] = 0 # Reset volume count
...
if __name__ == "__main__":
    if not os.path.exists(LOGS_PATH):
        ...
        extract_mimicry_cancal_features()
        print("\n[+] CanCal Mimicry Attack complete.")