...
def calculate_file_type_coverage(total_files_accessed, currently_seen_extensions, extension_counts_dict):
    """Calculates the ratio of files accessed vs total files of those extensions."""
...
def extract_split_ransomware_features():
    ...
    print(f"[*] Reading logs from: {ransomware_logs_path}")
    ...
    for session_name in os.listdir(ransomware_logs_path):
        ...
        # We need N independent states for N virtual processes
        split_states = []
        ...
        # Global counter to distribute operations (Round Robin)
        op_counter = 0
            ...
            with gzip.open(session_path, 'rt', encoding='utf-8') as fin:
                for line in fin:
                    try:
                        ...
                        # ONLY Process the Ransomware PID
                        if process_pid == ransomware_pid:
                            
                            # 1. Distribute this operation to one of the N virtual processes
                            target_split_idx = op_counter % N_SPLITS
                            state = split_states[target_split_idx]
                            change = False

                            # 2. Update stats for that SPECIFIC virtual process
                            ...
                            # 3. Update File Tracking for this virtual process
                            ...
                            # 4. Check TICKS for this virtual process
...
if __name__ == "__main__":
    print(f"--- Process Splitting Generator ---")
    if not os.path.exists(LOGS_PATH):
        print(f"[!] Error: Log path not found: {LOGS_PATH}")
    else:
        extract_split_ransomware_features()
